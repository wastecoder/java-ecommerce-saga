...
class ProcessedMessageIdTest {
	...
	void equalWhenSameFields() {
		ProcessedMessageId one = new ProcessedMessageId(EVENT_ID, "inventory.stock-command");
		ProcessedMessageId two = new ProcessedMessageId(EVENT_ID, "inventory.stock-command");

		assertThat(one).isEqualTo(two);
		assertThat(one).hasSameHashCodeAs(two);
		assertThat(one.getEventId()).isEqualTo(EVENT_ID);
		assertThat(one.getConsumer()).isEqualTo("inventory.stock-command");
	}
	...
	void notEqualWhenAnyFieldDiffers() {
		ProcessedMessageId base = new ProcessedMessageId(EVENT_ID, "inventory.stock-command");

		assertThat(base).isNotEqualTo(new ProcessedMessageId(OTHER_EVENT_ID, "inventory.stock-command"));
		assertThat(base).isNotEqualTo(new ProcessedMessageId(EVENT_ID, "inventory.other-consumer"));
