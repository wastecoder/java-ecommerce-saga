...
class KafkaEnvelopeRoundTripIntegrationTest {
	...
	void awaitAssignment() {
		// inventory.events is declared with 3 partitions; wait until the capture consumer owns them so the
	...
	@Test
	@DisplayName("Given an envelope published to inventory.events, when it is consumed, then the key, fields and payload round-trip with no Spring type headers")
	void publishAndConsume_roundTrips() throws InterruptedException {
		...
		publisher.publish("inventory.events", orderId, "StockReserved", payload);

		// The broker is shared across the suite, so inventory.events also carries other tests' replies;
		...
		assertThat(record).as("a record for this order should be consumed from inventory.events").isNotNull();
		...
		assertThat(envelope.type()).isEqualTo("StockReserved");
	...
	static class TestListenerConfig {
		...
		@KafkaListener(id = CAPTURE_ID, topics = "inventory.events", groupId = "inventory-roundtrip-test")
