 ...
 *
 * <p>When called inside a transaction (the events/commands emitted by the place-order use case and the
 * saga coordinator), the actual send is deferred to {@code afterCommit}: the broker never sees an
 * event/command for state that later rolls back, and a redelivery cannot observe an event whose dedup
 * marker has not committed yet. Outside a transaction the envelope is sent immediately. The envelope itself
 * is built eagerly so {@code occurredAt} reflects when the event happened, not when the commit flushed it.
 *
