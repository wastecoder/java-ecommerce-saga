...
/**
 * Makes payment-service an OAuth2 resource server: reading a payment ({@code GET /payments/**}) requires a
 * valid Keycloak JWT (any realm role); the operational (actuator) and documentation (Swagger) endpoints stay
 * open. The JWT is validated against the issuer configured in {@code application.yaml}
