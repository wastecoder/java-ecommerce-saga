...
/**
 * Blocking retry + dead-letter wiring for the {@code order.events} consumer. Spring Boot auto-associates
 * this single {@link DefaultErrorHandler} bean with the auto-configured listener container factory, so no
 * custom factory is needed. After the fixed retries — or immediately, for non-retryable errors — the record
 * is published to {@code <topic>.DLT}.
 */
@Configuration
public class KafkaErrorHandlingConfig {
	...
	DefaultErrorHandler kafkaErrorHandler(ProducerFactory<Object, Object> producerFactory) {
		...
		// A malformed event payload (ObjectMapper.convertValue) is deterministic — route straight to the DLT.
	 ...
	 * {@code KafkaTemplate}/{@code ProducerFactory} are preserved. It reuses the auto producer config (correct
	 * broker address, including under Testcontainers) and routes the value by type: a normal
	 * {@link EventEnvelope} is written as JSON without type headers, while a poison-pill value (surfaced by
	 * {@code ErrorHandlingDeserializer} as raw {@code byte[]}) passes through untouched.
	 */
