...
class ProcessedMessageIdTest {
	...
	void equalWhenSameFields() {
		ProcessedMessageId one = new ProcessedMessageId(EVENT_ID, "notification.order-event");
		ProcessedMessageId two = new ProcessedMessageId(EVENT_ID, "notification.order-event");

		assertThat(one).isEqualTo(two);
		assertThat(one).hasSameHashCodeAs(two);
		assertThat(one.getEventId()).isEqualTo(EVENT_ID);
		assertThat(one.getConsumer()).isEqualTo("notification.order-event");
	}
	...
	void notEqualWhenAnyFieldDiffers() {
		ProcessedMessageId base = new ProcessedMessageId(EVENT_ID, "notification.order-event");

		assertThat(base).isNotEqualTo(new ProcessedMessageId(OTHER_EVENT_ID, "notification.order-event"));
		assertThat(base).isNotEqualTo(new ProcessedMessageId(EVENT_ID, "notification.other-consumer"));
